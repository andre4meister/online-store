...
const DashboardProvider = ({ children }) => {
  ...
  const discountItem = dataCopy
    .filter((item) => item.discountPrice)
    .sort((a, b) => a.discountPrice - b.discountPrice)[0];
  const newItem = dataCopy
    .filter((item) => item)
    .sort((a, b) => new Date().toISOString(a.createdAt) - new Date().toISOString(b.createdAt))[0];
